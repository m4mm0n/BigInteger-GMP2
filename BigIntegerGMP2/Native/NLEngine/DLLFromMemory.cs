...
namespace BigIntegerGMP2.Native.NLEngine
{
    /// <summary>
    /// https://github.com/schellingb/DLLFromMemory-net/blob/master/DLLFromMemory.cs
    /// </summary>
    public class DLLFromMemory : IDisposable
    {
        ...
        delegate bool DllEntryDelegate(nint hinstDLL, DllReason fdwReason, nint lpReserved);

        [UnmanagedFunctionPointer(CallingConvention.Winapi)]
        delegate int ExeEntryDelegate();

        [UnmanagedFunctionPointer(CallingConvention.Winapi)]
        delegate void ImageTlsDelegate(nint dllHandle, DllReason reason, nint reserved);
        ...
        public nint GetPtrFromFuncName(string funcName)
        {
        ...
        void MemoryLoadLibrary(byte[] data)
        {
            ...
            nint oldHeader_OptionalHeader_ImageBase;
            if (Is64BitProcess) oldHeader_OptionalHeader_ImageBase = (nint)unchecked((long)OrgNTHeaders.OptionalHeader.ImageBaseLong);
            else oldHeader_OptionalHeader_ImageBase = unchecked((int)(OrgNTHeaders.OptionalHeader.ImageBaseLong >> 32));

            // reserve memory for image of library
            pCode = Win.VirtualAlloc(oldHeader_OptionalHeader_ImageBase, OrgNTHeaders.OptionalHeader.SizeOfImage, AllocationType.RESERVE | AllocationType.COMMIT, MemoryProtection.READWRITE);
            //pCode = IntPtr.Zero; //test relocation with this

            // try to allocate memory at arbitrary position
            if (pCode == nint.Zero) pCode = Win.VirtualAlloc(nint.Zero, OrgNTHeaders.OptionalHeader.SizeOfImage, AllocationType.RESERVE | AllocationType.COMMIT, MemoryProtection.READWRITE);

            if (pCode == nint.Zero) throw new DllException("Out of Memory");
            ...
            if (Is64BitProcess && PtrSpanBoundary(pCode, alignedImageSize, 32))
            {
                ...
                while (PtrSpanBoundary(pCode, alignedImageSize, 32))
                {
                    ...
                    pCode = Win.VirtualAlloc(nint.Zero, alignedImageSize, AllocationType.RESERVE | AllocationType.COMMIT, MemoryProtection.READWRITE);
                    if (pCode == nint.Zero) break;
                }
            ...
            var headers = Win.VirtualAlloc(pCode, OrgNTHeaders.OptionalHeader.SizeOfHeaders, AllocationType.COMMIT, MemoryProtection.READWRITE);
            ...
            _isRelocated = locationDelta != nint.Zero ? PerformBaseRelocation(ref OrgNTHeaders, pCode, locationDelta) : true;
            ...
            if (OrgNTHeaders.OptionalHeader.AddressOfEntryPoint != 0)
            {
                if (IsDll)
                {
                    ...
                    _initialized = _dllEntry != null && _dllEntry(pCode, DllReason.DLL_PROCESS_ATTACH, nint.Zero);
        ...
        static void CopySections(ref IMAGE_NT_HEADERS OrgNTHeaders, nint pCode, nint pNTHeaders, byte[] data)
        {
            ...
            for (var i = 0; i < OrgNTHeaders.FileHeader.NumberOfSections; i++, pSection = PtrAdd(pSection, Sz.IMAGE_SECTION_HEADER))
            {
                ...
                if (Section.SizeOfRawData == 0)
                {
                    ...
                    if (size > 0)
                    {
                        var dest = Win.VirtualAlloc(PtrAdd(pCode, Section.VirtualAddress), size, AllocationType.COMMIT, MemoryProtection.READWRITE);
                ...
                else
                {
                    ...
                    var dest = Win.VirtualAlloc(PtrAdd(pCode, Section.VirtualAddress), Section.SizeOfRawData, AllocationType.COMMIT, MemoryProtection.READWRITE);
        ...
        static bool PerformBaseRelocation(ref IMAGE_NT_HEADERS OrgNTHeaders, nint pCode, nint delta)
        {
        ...
        static nint[] BuildImportTable(ref IMAGE_NT_HEADERS OrgNTHeaders, nint pCode)
        {
            ...
            for (uint i = 0; i != NumEntries; i++, pImportDesc = PtrAdd(pImportDesc, Sz.IMAGE_IMPORT_DESCRIPTOR))
            {
                ...
                for (var SzRef = nint.Size; ; pThunkRef = PtrAdd(pThunkRef, SzRef), pFuncRef = PtrAdd(pFuncRef, SzRef))
                {
                    ...
                    WriteFuncRef = Win.GetProcAddress(handle,
                        Win.IMAGE_SNAP_BY_ORDINAL(ReadThunkRef)
                            ? Win.IMAGE_ORDINAL(ReadThunkRef)
                            : PtrAdd(PtrAdd(pCode, ReadThunkRef), Of.IMAGE_IMPORT_BY_NAME_Name));
        ...
        static void FinalizeSections(ref IMAGE_NT_HEADERS OrgNTHeaders, nint pCode, nint pNTHeaders, uint PageSize)
        {
        ...
        static void ExecuteTLS(ref IMAGE_NT_HEADERS OrgNTHeaders, nint pCode, nint pNTHeaders)
        {
            ...
            if (pCallBack != nint.Zero)
            {
                for (nint Callback; (Callback = PtrRead<nint>(pCallBack)) != nint.Zero; pCallBack = PtrAdd(pCallBack, nint.Size))
                {
                    ...
                    tls(pCode, DllReason.DLL_PROCESS_ATTACH, nint.Zero);
                }
            }
        }
        ...
        public static bool Is64BitProcess { get { return nint.Size == 8; } }

        static uint GetMachineType() { return nint.Size == 8 ? Win.IMAGE_FILE_MACHINE_AMD64 : Win.IMAGE_FILE_MACHINE_I386; }

        static uint AlignValueUp(uint value, uint alignment) { return value + alignment - 1 & ~(alignment - 1); }

        static nint GetRealSectionSize(ref IMAGE_SECTION_HEADER Section, ref IMAGE_NT_HEADERS NTHeaders)
        {
        ...
        public void Dispose()
        {
            if (_initialized)
            {
                if (_dllEntry != null) _dllEntry.Invoke(pCode, DllReason.DLL_PROCESS_DETACH, nint.Zero);
                _initialized = false;
            }

            if (ImportModules != null)
            {
                foreach (var m in ImportModules) if (!PtrIsInvalidHandle(m)) Win.FreeLibrary(m);
                ImportModules = null;
            }

            if (pCode != nint.Zero)
            {
                Win.VirtualFree(pCode, nint.Zero, AllocationType.RELEASE);
                pCode = nint.Zero;
                pNTHeaders = nint.Zero;
            }
        ...
        class Win
        {
            ...
            public static nint IMAGE_FIRST_SECTION(nint pNTHeader, ushort ntheader_FileHeader_SizeOfOptionalHeader)
            {
                return PtrAdd(pNTHeader, Of.IMAGE_NT_HEADERS_OptionalHeader + ntheader_FileHeader_SizeOfOptionalHeader);
            }
        ...
        static T PtrRead<T>(nint ptr) { return (T)Marshal.PtrToStructure(ptr, typeof(T)); }
        static void PtrWrite<T>(nint ptr, T val) { Marshal.StructureToPtr(val, ptr, false); }
        static nint PtrAdd(nint p, int v) { return (nint)(p.ToInt64() + v); }
        static nint PtrAdd(nint p, uint v) { return nint.Size == 8 ? (nint)(p.ToInt64() + unchecked(v)) : p.ToInt32() + unchecked((int)v); }
        static nint PtrAdd(nint p, nint v) { return nint.Size == 8 ? (nint)(p.ToInt64() + v.ToInt64()) : p.ToInt32() + v.ToInt32(); }
        static nint PtrAdd(nint p, nuint v) { return nint.Size == 8 ? (nint)(p.ToInt64() + unchecked((long)v.ToUInt64())) : p.ToInt32() + unchecked((int)v.ToUInt32()); }
        static nint PtrSub(nint p, nint v) { return nint.Size == 8 ? (nint)(p.ToInt64() - v.ToInt64()) : p.ToInt32() - v.ToInt32(); }
        static nint PtrBitOr(nint p, nuint v) { return nint.Size == 8 ? (nint)unchecked((long)(unchecked((ulong)p.ToInt64()) | v.ToUInt64())) : unchecked((int)(unchecked((uint)p.ToInt32()) | v.ToUInt32())); }
        static nint PtrAlignDown(nint p, nuint align) { return (nint)unchecked((long)(unchecked((ulong)p.ToInt64()) & ~(align.ToUInt64() - 1))); }
        static bool PtrIsInvalidHandle(nint h) { return h == nint.Zero || h == (nint.Size == 8 ? (nint)(long)-1 : -1); }
        static bool PtrSpanBoundary(nint p, uint Size, int BoundaryBits) { return unchecked((ulong)p.ToInt64()) >> BoundaryBits < unchecked((ulong)p.ToInt64()) + Size >> BoundaryBits; }
